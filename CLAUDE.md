...
모든 작업에 **질문 → 결정 → 실행 → 코드 리딩 → 회고** 사이클을 적용한다.
...
모든 작업에 **질문 → 결정 → 실행 → 코드 리딩 → 회고** 사이클을 적용한다. 예외 없음.
...
| 3 | **코드 리딩** | 변경 코드 이해도 점검. 기본 레벨 C(응용). "바빠"→B, "급해"→A |
...
| 스킬 | 용도 |
|------|------|
| `code-reading` | 작업 전후 코드 이해도 점검 (사전 점검 + 사후 코드 리딩) |
...
**단순** → Thinking Cycle(질문→결정→실행→코드 리딩→회고) 적용 후 결과 보고.
...
#### Phase 5: 코드 리딩 (Thinking Cycle Phase 3)

보고 후, 변경된 코드에 대해 사용자의 이해도를 점검한다. 레벨 C(기본)로 What → Why → 응용 질문.
코드 변경이 없는 작업은 생략.

#### Phase 6: 회고 (Thinking Cycle Phase 4)

코드 리딩 후 사용자에게 회고 질문을 던진다. 사용자가 답하면 `.claude/reflections/YYYY-MM-DD.md`에 기록.
회고 없이 작업을 종료하지 않는다.
