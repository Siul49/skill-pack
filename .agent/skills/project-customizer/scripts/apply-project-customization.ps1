param(
    [string]$ConfigPath = ".agent/project-customization.json",
    [switch]$Init,
...
function New-VerifySkillMarkdown {
    ...
    [void]$sb.AppendLine("---")
    [void]$sb.AppendLine()
    [void]$sb.AppendLine("# $($name -replace '-', ' ' -replace '^verify ', 'Verify ')")
    [void]$sb.AppendLine()
...
$configObj = Get-Content -Raw $resolvedPath | ConvertFrom-Json
$config = @{}
...
Insert-RowIntoTable `
    ...
    -RowGenerator {
        ...
        return ("| $next | $bt$verifySkillName$bt | $($config.execution_description) |")
    }

Insert-RowIntoTable `
    -FilePath $manageSkillsFile `
    -HeaderPattern '^\|\s*Skill\s*\|\s*Description\s*\|\s*Coverage patterns\s*\|' `
    -Row ("| $bt$verifySkillName$bt | $($config.execution_description) | $bt" + ($config.coverage_patterns -join "$bt, $bt") + "$bt |") `
    -DuplicatePattern [regex]::Escape("$bt$verifySkillName$bt")
...
Insert-RowIntoTable `
    ...
    -Row "| $routingKeywordsText | **$verifySkillName** | Project-specific verify skill |" `
    -DuplicatePattern [regex]::Escape("**$verifySkillName**")

Insert-RowIntoTable `
    -FilePath $routingFile `
    -HeaderPattern '^\|\s*Skill\s*\|\s*Description\s*\|' `
    -Row "| $verifySkillName | $($config.execution_description) |" `
    -DuplicatePattern [regex]::Escape("| $verifySkillName |")
