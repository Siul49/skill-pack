...
function Get-TaskPaths {
    ...
    $taskRoot = Join-Path (Join-Path $Root ".agent/context") $Id
    return @{
        Root       = $taskRoot
...
function Resolve-CurrentTaskId {
    ...
    $currentFile = Join-Path (Join-Path $Root ".agent/context") "current-task.txt"
...
function Set-CurrentTaskId {
    param([string]$Root, [string]$Id)
    $ctxRoot = Join-Path $Root ".agent/context"
